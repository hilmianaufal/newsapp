							...
							<div class="card full-height">
								<div class="card-header">
									<div class="d-flex align-items-center">
										...
										<a class="btn btn-primary btn-round ml-auto" href="{{ route('materi.create') }}">
									...
									<div class="table-responsive">
										<table id="add-row" class="display table table-striped table-hover table-bordered" >
											...
											<tbody>
                                                ...
                                                @forelse  ($materi as $ply)
												<tr>
                                                    <td>{{ $loop->iteration }}</td>
													<td>{{ $ply->judul_materi }}</td>
													<td>{{ $ply->slug }}</td>
													<td>{{ $ply->link }}</td>
                                                    <td>
													 @if ($ply->is_active == 1)
													...
													<td>
														<div class="form-button-action">
                                                            <a type="button" href="{{ route('materi.edit', $ply->id) }}"  title="edit" class="btn btn-link btn-primary" >
															 ...
															 @method('DELETE')
                                                              <form action="{{ route('materi.destroy', $ply->id) }}" method="POST" style="display:inline;">
                                                                    @csrf
                                                                    @method('DELETE')
